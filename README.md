# Portfólio Acadêmico | Lucas Jesus Custódio Silva

Bem-vindo ao meu portfólio acadêmico de Engenharia de Software.

[**Acessar o portfólio publicado**](https://lucaasjs-dev.github.io)

## Apresentação

Sou um profissional com experiência em negócios, projetos, produtos e melhoria de processos. Minha trajetória combina visão de gestão com formação tecnológica, buscando transformar necessidades de negócio em soluções digitais organizadas, eficientes e mensuráveis.

## Sobre mim

- Graduando em Engenharia de Software.
- Tecnólogo em Processos Gerenciais.
- Experiência com negociação, planejamento e acompanhamento de projetos.
- Interesse em gestão de processos, produtos digitais e liderança.
- Objetivo profissional: evoluir para posições de gerência de processos e direção.

## Projetos

| Projeto | Descrição | Competências aplicadas |
|---|---|---|
| [Sistema Completo de Gestão de Estoque](https://lucaasjs-dev.github.io/#gestao-estoque) | Plataforma para controlar entradas, saídas, transferências, inventários, níveis mínimos e rastreabilidade das movimentações. | Regras de negócio, modelagem de dados e indicadores |
| [Painel de Indicadores Operacionais](https://lucaasjs-dev.github.io/#painel-indicadores) | Proposta de painel para acompanhar prazos, produtividade, volume de trabalho e gargalos. | Business Intelligence, indicadores e tomada de decisão |
| [Redesenho de Processo de Atendimento](https://lucaasjs-dev.github.io/#processo-atendimento) | Mapeamento do fluxo atual e desenho de um processo futuro mais padronizado e rastreável. | BPM, requisitos, melhoria contínua e gestão de processos |

## Habilidades e tecnologias

- Mapeamento e análise de processos
- Engenharia de requisitos e regras de negócio
- Gestão de projetos e metodologias ágeis
- Gestão de produtos e priorização de demandas
- Business Intelligence e indicadores de desempenho
- Git e GitHub
- Lógica de programação e banco de dados
- Liderança, negociação e comunicação entre equipes

## Formação

- **Bacharelado em Engenharia de Software** — UNICID, cursando.
- **MBA em Gestão de Projetos e Metodologias Ágeis** — PUCRS, em andamento.
- **Tecnologia em Processos Gerenciais** — Universidade Metodista de São Paulo, concluído em 2026.
- **Técnico em Automação Industrial** — concluído em 2016.

## Contato

- [LinkedIn](https://www.linkedin.com/in/lucaasjs/)
- [GitHub](https://github.com/lucaasjs-dev)
- E-mail: lucaas.js@gmail.com

---

Este repositório concentra atividades, estudos e projetos desenvolvidos durante minha formação em Engenharia de Software.
